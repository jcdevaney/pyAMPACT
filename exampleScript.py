...
from getPitchVibratoData import get_pitch_vibrato_data
...
# alignment_visualizer(select_state, midi_data, spec, 1) # Original
alignment_visualizer(select_state, midi_file, spec, 1)

# Get onset and offset times
onsets, offsets = get_ons_offs(select_state)

# Write the onset and offset times to a file
with open('example.txt', 'w') as f:
    for onset, offset in zip(onsets, offsets):
        f.write(f"{onset}\t{offset}\n")

# Load corrected labels
fixed_labels = np.loadtxt('exampleFixed.txt')
onsets = fixed_labels[:, 0]
offsets = fixed_labels[:, 1]


# Map timing information to the quantized MIDI file
# nmat_new = get_timing_data(midi_file, onsets, offsets)
nmat_new = get_timing_data(midi_file, onsets)
...
# cents = get_cent_vals(onsets, offsets, yin_res) # Original
cents = get_cent_vals(onsets, yin_res, target_sr)  # Original
...
#     min_idx, max_idx = find_peaks(cents[i], 100, int(target_sr/32), 60)
#     mins.append(min_idx)
